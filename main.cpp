...
inline CoroutineTaskQueue::Task
addCoroutineTask(CoroutineTaskQueue &queue, std::function<void()> callback) {
  callback();
...
inline CoroutineTaskQueue::Task
addCoroutineTask(CoroutineTaskQueue &queue,
...
template <class T>
inline CoroutineTaskQueue::Task
addCoroutineTask(CoroutineTaskQueue &queue, std::function<void(T)> callback,
                 T data) {
  callback(data);
  // Yield back to queue so other tasks can run.
  co_await queue.requeue();
...
inline CoroutineTaskQueue::Task
addCoroutineTaskWithResult(CoroutineTaskQueue &queue,
...
int main() {
  ...
  queue.enqueue(addCoroutineTask(
      ...
      [](const std::vector<uint8_t> &data) {
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
        safePrint("[addCoroutineTask<vector<uint8_t>>] size=" +
  ...
  queue.enqueue(addCoroutineTaskWithResult<int>(
      queue, []() { return 7 * 6; }, resultPromise));
  std::this_thread::sleep_for(std::chrono::milliseconds(100));
  safePrint("[addCoroutineTaskWithResult] result=" +
            std::to_string(resultFuture.get()));
