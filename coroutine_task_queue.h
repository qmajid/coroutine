  ...
  public:
    ...
    class RequeueAwaitable {
      public:
        ...
        void await_suspend(std::coroutine_handle<> handle) const { queue_.enqueue(handle); }
    ...
    explicit CoroutineTaskQueue(std::size_t workerCount = std::thread::hardware_concurrency());
    ~CoroutineTaskQueue();
